AC_DEFUN([WITH_LIBLSB],
    ...
    if test x"${with_liblsb}" == xyes; then
        ...
        AC_CHECK_HEADER(liblsb.h, [liblsb_path=${with_liblsb}; liblsb_found=yes], [AC_MSG_ERROR([Can't find the libLSB header files in ${with_liblsb}])])
    fi

    if test x"${liblsb_found}" == xyes; then
        AC_DEFINE(HAVE_LIBLSB, 1, enables libLSB)
        AC_MSG_NOTICE([libLSB support enabled])
        if test x${liblsb_path} != x; then
            CXXFLAGS="${CXXFLAGS} -I${with_liblsb}/"
            LDFLAGS="${LDFLAGS} -L${liblsb_path}/lib"
        fi
        LIBS="${LIBS} -llsb"
    fi
    ]
