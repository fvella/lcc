...
static void init_lcc_1degree(uint64_t *edrem, uint64_t nedrem, uint64_t nverts, LOCINT * reach)
{
	uint64_t i;
	LOCINT ur = 0;
	for (i = 0; i < nedrem; i++){
...
#ifdef CLAMPI
					gres = CMPI_Get(adj_v, r_off[1]-r_off[0], MPI_UINT32_T, dest_get, r_off[0], r_off[1]-r_off[0], MPI_UINT32_T, win_row);
					if (gres!=CL_HIT) CMPI_Win_flush(dest_get, win_row);
#else
					MMPI_GET(adj_v, r_off[1]-r_off[0], MPI_UINT32_T, dest_get, r_off[0], r_off[1]-r_off[0], MPI_UINT32_T, win_row.win);
					MMPI_WIN_FLUSH(dest_get, win_row.win);
#endif
...
#define CHECKRTYPE(exitval,opt) {\
		...
		switch (c) {
			...
			case 'H' :
					...
					if ( heuristic >= 2) prexit("H2 and H3 heuristics are not implemented(-H): %s\n", optarg);
					break;
					//heuristic selection
			case 'o' :
					ofile = strdup(optarg);
			...
			case 'E':
					...
					break;
			case 'N':
					if (0 == sscanf(optarg, "%ld", &nbfs)) prexit("Option not implemented (-N): %s\n", optarg);
					break;
			case 'U':
					// Undirected
					undirected = 0;
...
#undef CHECKRTYPE
	...
	dist_lcc =   (float*)Malloc(col_bl*sizeof(float));// Local Cluster Coef... Store in scan mode
	if (heuristic == 1){
